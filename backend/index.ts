...
import statusFormRoutes from './routes/statusFormRoutes.js';
...
import './workers/userDeactivationWorker.js';
...
// Unlike /health (a cached flag, deliberately never touches the DB so the
// keep-alive ping stays cheap), this runs a real round-trip — the frontend
// fires it as soon as the login page mounts, to unpause a free-tier Postgres
// provider (e.g. Supabase) that's gone idle, ahead of the user actually
...
app.use('/api/status-forms', statusFormRoutes);
...
async function start() {
  ...
  mongoose
    ...
    .catch((err: any) => console.error('MongoDB connection error (non-fatal):', err.message));
