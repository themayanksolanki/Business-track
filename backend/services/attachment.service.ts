...
type AccessTask = { id: number; organizationId: number | null; assignedToId: number; createdById: number };
...
export const getAttachmentDownloadInfo = async (attachment: Attachment) => {
...
export const purgeExpiredFor = async (where: { taskId: number } | { projectItemId: number }) => {
...
export const listAttachments = (where: { taskId: number } | { projectItemId: number } | { projectId: number }) =>
...
export async function loadAttachmentOrFail(
  where: { id: number; taskId: number } | { id: number; projectItemId: number } | { id: number; projectId: number }
) {
...
export async function loadTaskForAttachments(taskId: number, user: AuthUser): Promise<AccessTask> {
  const task = await prisma.task.findUnique({ where: { id: taskId } });
  if (!task) throw new AppError('Task not found', 404);
  if ((await getTaskAccessLevel(task, user)) !== 'edit') throw new AppError('Access denied', 403);
  return task;
}
...
export async function loadProjectForAttachments(projectId: number, user: AuthUser, opts?: { requireEdit?: boolean }) {
...
export async function loadItemForAttachments(projectId: number, itemId: number) {
  const item = await loadItemOrFail(projectId, itemId);
  if (item.type === 'group') throw new AppError('Groups do not support attachments', 400);
  return item;
}

type UploadedFile = { originalname: string; path: string; filename: string; mimetype: string; size: number };
...
export async function createFileAttachment(opts: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (opts.taskId) await tx.task.update({ where: { id: opts.taskId }, data: { attachmentCount: { increment: 1 } } });
...
export async function createLinkAttachment(opts: { projectItemId: number; url: string; fileName: string; uploadedById: number }) {
  return prisma.$transaction(async (tx) => {
    const created = await tx.attachment.create({
      data: {
        projectItemId: opts.projectItemId,
...
export async function scheduleAttachmentDelete(attachment: Attachment) {
  ...
  if (attachment.pendingDeleteAt && attachment.pendingDeleteAt > new Date()) {
    return { attachment, alreadyPending: true as const };
  }
  ...
  return { attachment: updated, alreadyPending: false as const };
}

export async function undoAttachmentDelete(attachment: Attachment) {
  if (!attachment.pendingDeleteAt || attachment.pendingDeleteAt <= new Date())
...
export async function hardDeleteAttachmentNow(attachment: Attachment) {
  await destroyBlob(attachment);
  await prisma.attachment.delete({ where: { id: attachment.id } });
}
