...
export const sameOrg = (a: number | null | undefined, b: number | null | undefined) => (a ?? null) === (b ?? null);
...
export async function loadOrgScopedOrFail<T extends { organizationId: number | null }>(
  find: () => Promise<T | null>,
  organizationId: number | null,
  notFoundMessage = 'Not found'
): Promise<T> {
  const row = await find();
  if (!row || !sameOrg(row.organizationId, organizationId)) throw new AppError(notFoundMessage, 404);
  return row;
}
