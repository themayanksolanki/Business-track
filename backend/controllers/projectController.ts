...
const shapeProject = <T extends PlanFields>(p: T) => {
...
export const getSharedProject = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const hasNormalAccess = await canAccessProject(req.user! as AuthUser, project);
...
export const updateProject = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!(await canAccessProject(req.user! as AuthUser, project)))
      return next(new AppError('You do not have access to this project', 403));
    if (!canManageProjectSettings(req.user! as AuthUser, project))
    ...
    if (project.status === 'draft') {
      ...
      if (status === 'active' && !canApproveDraft(req.user! as AuthUser, project))
        return next(new AppError("Only an Admin or this draft's creator can approve it", 403));
    }

    const data: Prisma.ProjectUncheckedUpdateInput = { updatedById: req.user!.id };
    ...
    if (links !== undefined) data.links = links.map((l: { title: string; url: string }) => ({ title: l.title.trim(), url: l.url.trim() }));
    ...
    if (ownerChanged && data.ownerId) {
      await notifyUser(data.ownerId as number, req.user!.id, {
        type: 'projectAssigned',
        title: 'Assigned to a project',
...
export const updateProjectDetailsLayout = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const project = await prisma.project.findUnique({
      where: { id: Number(req.params.projectId) },
      include: ACCESS_INCLUDE,
...
export const uploadProjectPlan = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const project = await prisma.project.findUnique({
      where: { id: Number(req.params.projectId) },
      include: ACCESS_INCLUDE,
...
export const downloadProjectPlan = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const project = await prisma.project.findUnique({
      where: { id: Number(req.params.projectId) },
      include: ACCESS_INCLUDE,
...
export const removeProjectPlan = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const project = await prisma.project.findUnique({
      where: { id: Number(req.params.projectId) },
      include: ACCESS_INCLUDE,
