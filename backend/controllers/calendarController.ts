...
import { canAccessCalendar, type AuthUser } from './eventController.js';
...
export const getCalendars = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(200).json(
      calendars.map(({ _count, ...calendar }) => ({ ...calendar, eventsCount: _count.events }))
    );
  } catch (err) {
    next(err);
  }
...
export const createCalendar = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (existing) return next(new AppError('A calendar with this name already exists', 409));
    ...
    if (err.code === 'P2002') return next(new AppError('A calendar with this name already exists', 409));
    next(err);
  }
};
...
export const updateCalendar = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const data: Prisma.CalendarUncheckedUpdateInput = { updatedById: req.user!.id };
