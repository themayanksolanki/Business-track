...
const getViewableDepartmentIds = (user: AuthUser) =>
  user.role === 'Admin' || user.role === 'Manager' ? Promise.resolve(null) : getAccessibleDepartmentIds(user);

export const getDepartments = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const accessibleIds = await getViewableDepartmentIds(req.user! as AuthUser);

    if (req.query.page === undefined) {
      const where: Prisma.DepartmentWhereInput = { organizationId: req.user!.organizationId };
    ...
    const rootWhere: Prisma.DepartmentWhereInput = { organizationId: req.user!.organizationId, parentId: null };
...
export const updateDepartment = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const data: Prisma.DepartmentUncheckedUpdateInput = { updatedById: req.user!.id };
