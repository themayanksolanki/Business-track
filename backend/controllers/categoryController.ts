...
const getDescendantIds = async (rootIds: number | number[]): Promise<number[]> => {
  const result: number[] = [];
...
const withCounts = <T extends { _count: { projects: number; children: number } }>(c: T) => {
  const { _count, ...rest } = c;
...
export const getCategories = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const rootWhere: Prisma.CategoryWhereInput = { organizationId: req.user!.organizationId, parentId: null };
...
export const updateCategory = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const data: Prisma.CategoryUncheckedUpdateInput = { updatedById: req.user!.id };
