...
export const ensureDefaultProjectRoles = async (organizationId: number, actorId: number) => {
...
export const getProjectRoles = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const createProjectRole = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const updateProjectRole = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const deleteProjectRole = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const role = await prisma.projectRole.findUnique({ where: { id: Number(req.params.id) } });
    if (!role || role.organizationId !== req.user!.organizationId)
      return next(new AppError('Role not found', 404));
...
export const reorderProjectRoles = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (numericIds.length !== roleIds.size || !numericIds.every((id: number) => roleIds.has(id)))
      return next(new AppError('orderedIds must match exactly the roles in this organization', 400));

    await prisma.$transaction(
      numericIds.map((id: number, index: number) => prisma.projectRole.update({ where: { id }, data: { rank: index } }))
    );
