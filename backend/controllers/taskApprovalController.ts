...
import type { ApprovalHistoryAction } from '@prisma/client';
...
const canManageApprovers = (
  user: AuthUser,
  item: { assignedToId: number | null; createdById: number },
  project: { createdById: number; ownerId: number | null }
) => user.id === item.assignedToId || user.id === item.createdById || canManageProjectSettings(user, project);

const logHistory = (projectItemId: number, userId: number, action: ApprovalHistoryAction, detail?: string) =>
  prisma.approvalHistory.create({ data: { projectItemId, userId, action, detail } });

const getActiveApprovers = (projectItemId: number) =>
...
async function loadContext(req: Request, next: NextFunction, withRole = false) {
...
export const getApprovers = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const assignApprovers = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const ctx = await loadContext(req, next, true);
    if (!ctx) return;
    const { project, item } = ctx;

    if (!canManageApprovers(req.user! as AuthUser, item, project))
    ...
    const userIds: number[] = Array.isArray(req.body.userIds) ? req.body.userIds.map(Number) : [];
    if (userIds.length === 0) return next(new AppError('userIds must be a non-empty array', 400));
    ...
    for (const userId of userIds) {
      ...
      if (existing && existing.status !== 'cancelled') continue; // already an active approver
...
export const removeApprover = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const ctx = await loadContext(req, next, true);
    if (!ctx) return;
    const { project, item } = ctx;
    ...
    if (!approver || approver.status === 'cancelled') return next(new AppError('Approver not found', 404));
...
export const requestChanges = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const ctx = await loadContext(req, next);
    if (!ctx) return;
    const { project, item } = ctx;
    ...
    if (!commentBody) return next(new AppError('A comment is required to request changes', 400));
...
export const reRequestApproval = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const ctx = await loadContext(req, next, true);
    if (!ctx) return;
    const { project, item } = ctx;
    ...
    if (toReset.length === 0) return next(new AppError('No approvers currently have changes requested', 400));
...
export const getApprovalComments = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const ctx = await loadContext(req, next);
    if (!ctx) return;
...
export const createApprovalComment = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const ctx = await loadContext(req, next, true);
    if (!ctx) return;
    const { project, item } = ctx;

    if (!canEditProject(req.user! as AuthUser, project))
    ...
    if (replyToId) {
      ...
      if (!parent) return next(new AppError('Comment being replied to was not found', 404));
    }
...
export const updateApprovalComment = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const ctx = await loadContext(req, next);
    if (!ctx) return;
    ...
    if (existing.authorId !== req.user!.id) return next(new AppError('You can only edit your own comments', 403));
...
export const deleteApprovalComment = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const ctx = await loadContext(req, next);
    if (!ctx) return;
    ...
    if (comment.authorId !== req.user!.id) return next(new AppError('You can only delete your own comments', 403));
