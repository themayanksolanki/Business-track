...
import type { User as PrismaUser, Organization as PrismaOrganization } from '@prisma/client';
...
export const refresh = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const decoded = jwt.verify(token, process.env.JWT_REFRESH_SECRET!) as { id: number | string };
    const user = await prisma.user.findUnique({
