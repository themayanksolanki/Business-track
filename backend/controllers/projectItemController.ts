...
const getDescendantIds = async (rootId: number): Promise<number[]> => {
  const result: number[] = [];
  let frontier = [rootId];
...
const getMaxDescendantDepth = async (rootId: number, rootDepth: number): Promise<number> => {
...
const shiftDescendantDepths = async (rootId: number, delta: number): Promise<void> => {
  let frontier = [rootId];
  while (frontier.length) {
    const children = await prisma.projectItem.findMany({
      where: { parentId: { in: frontier } },
...
type SubtreeSource = Pick<ProjectItem, 'id' | 'type' | 'title' | 'depth'>;
...
const attachApproverCounts = async <T extends { id: number }>(
  items: T[]
): Promise<(T & { approverCount: number; pendingApproverCount: number })[]> => {
  ...
  const byItem: Record<number, { approverCount: number; pendingApproverCount: number }> = {};
  for (const { projectItemId, status, _count } of counts) {
    if (!byItem[projectItemId]) byItem[projectItemId] = { approverCount: 0, pendingApproverCount: 0 };
    byItem[projectItemId].approverCount += _count._all;
    if (status === 'pending' || status === 'changesRequested') byItem[projectItemId].pendingApproverCount += _count._all;
  }
...
export const getItems = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (limit) {
      ...
      const parsedLimit = Math.min(100, Math.max(1, parseInt(limit as string, 10) || 12));
...
export const getSharedProjectItems = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const getItemsSummary = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const project = await prisma.project.findUnique({
      where: { id: Number(req.params.projectId) },
      include: ACCESS_INCLUDE,
...
export const updateItem = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (validMentions !== undefined) data.mentions = validMentions as unknown as Prisma.InputJsonValue;
    ...
    if (validMentions !== undefined) {
      const oldMentionIds = new Set(
        Array.isArray(item.mentions) ? (item.mentions as any[]).map((m) => m.userId) : []
      );
...
export const duplicateItem = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const project = await prisma.project.findUnique({
      where: { id: Number(req.params.projectId) },
      include: ACCESS_INCLUDE_WITH_ROLE,
...
// adjacent sibling.
export const moveItemToParent = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const project = await prisma.project.findUnique({
      where: { id: Number(req.params.projectId) },
      include: ACCESS_INCLUDE_WITH_ROLE,
...
// Re-homes an item (and its entire subtree) into a DIFFERENT project —
// groups move to the destination's root (they're always depth 0); tasks and
// subtasks need a targetParentId picked from the destination project's own
// tree. canAccessProject/canEditProject already reject cross-org projects,
// so organizationId/sequenceId never need to change here — only projectId
// does, cascaded across the whole subtree via a plain updateMany (the
// subtree's internal parentId links are untouched; only the moved item's
// own parentId points at the new external parent).
export const moveItemToProject = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const project = await prisma.project.findUnique({
      where: { id: Number(req.params.projectId) },
      include: ACCESS_INCLUDE_WITH_ROLE,
    ...
    if (targetProjectId === project.id)
      return next(new AppError('Item is already in this project', 400));
    ...
    if (item.type === 'group') {
      if (targetParentId != null)
        return next(new AppError('Groups always move to the root of the target project', 400));
    } else {
      if (!targetParentId) return next(new AppError('targetParentId is required for this item', 400));
      newParent = await prisma.projectItem.findFirst({
        where: { id: Number(targetParentId), projectId: targetProjectId },
      });
      if (!newParent) return next(new AppError('Target item not found in the target project', 404));
      newDepth = newParent.depth + 1;
    }
    ...
    // append to the end of the destination's siblings
    const newSiblingCount = await prisma.projectItem.count({
      where: { projectId: targetProjectId, parentId: newParentId },
    });
    ...
    const updated = await prisma.projectItem.findUnique({ where: { id: item.id }, include: ITEM_INCLUDE });
    res.status(200).json({ message: 'Item moved to project', item: updated });
  } catch (err) {
    next(err);
  }
};
...
export const bulkMoveItemsToParent = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const project = await prisma.project.findUnique({
      where: { id: Number(req.params.projectId) },
      include: ACCESS_INCLUDE_WITH_ROLE,
...
export const reorderItems = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (
      ...
      !numericOrderedIds.every((id: number) => siblingIds.has(id))
    ) {
      return next(new AppError('orderedIds must match exactly the siblings of this parent', 400));
    }

    await prisma.$transaction(
      numericOrderedIds.map((id: number, index: number) => prisma.projectItem.update({ where: { id }, data: { order: index } }))
    );
