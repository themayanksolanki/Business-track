...
export const createComment = async (req, res, next) => {
  try {
    ...
    await notifyUsers([item.assignedToId, item.createdById].filter(Boolean), req.user.id, {
...
export const updateComment = async (req, res, next) => {
  try {
    ...
    const comment = await prisma.comment.update({
      ...
      data: { body: req.body.body.trim(), mentions },
      include: { author: { select: AUTHOR_SELECT } },
    });

    const oldMentionIds = new Set(
      Array.isArray(existing.mentions) ? existing.mentions.map((m) => m.userId) : []
    );
