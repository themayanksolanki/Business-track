...
export const canAccessEvent = (user, event) => {
...
export const canEditEvent = (user, event) => user.role === 'Admin' || event.ownerId === user.id;
...
export const canAccessCalendar = (user, calendar) => calendar.organizationId === user.organizationId && (user.role === 'Admin' || calendar.ownerId === user.id);
const guestsCreateData = (guests) => guests.map((g) => ({
    email: g.email.trim().toLowerCase(),
    name: g.name || null,
    userId: g.userId ? Number(g.userId) : null,
}));
const remindersCreateData = (reminders) => reminders.map((r) => ({
    method: r.method ?? 'notification',
    minutesBefore: r.minutesBefore ?? 10,
}));
const recurrenceCreateData = (recurrence) => ({
...
async function expandEventsToOccurrences(events, rangeStart, rangeEnd) {
    ...
    for (const master of recurringMasters) {
        if (!master.recurrence)
            continue; // recurrenceId set implies this exists; defensive only
    ...
    return [...nonRecurringItems, ...occurrenceItems].sort((a, b) => new Date(a.start).getTime() - new Date(b.start).getTime());
}
...
export const createEvent = async (req, res, next) => {
    try {
        ...
        const event = await prisma.$transaction(async (tx) => {
            const resolvedCalendar = calendar ?? (await getOrCreateDefaultCalendar(tx, req.user));
...
export const updateEvent = async (req, res, next) => {
    try {
        ...
        if (calendarId !== undefined)
            data.calendarId = Number(calendarId);
        if (visibility !== undefined)
            data.visibility = visibility;
        if (busyStatus !== undefined)
            data.busyStatus = busyStatus;
            ...
            if (guests !== undefined) {
                ...
                if (guests.length) {
                    await tx.guest.createMany({
                        data: guestsCreateData(guests).map((g) => ({
                            ...g,
                            eventId: event.id,
                        })),
                    });
            ...
            if (reminders !== undefined) {
                ...
                if (reminders.length) {
                    await tx.eventReminder.createMany({
                        data: remindersCreateData(reminders).map((r) => ({
                            ...r,
                            eventId: event.id,
                        })),
                    });
...
export const deleteEvent = async (req, res, next) => {
    try {
        ...
        await prisma.$transaction([
            ...modifiedExceptions
                .filter((ex) => ex.overrideEventId !== null)
...
// stop here.
async function loadRecurringMasterForOccurrence(eventId, next, include) {
    const event = await prisma.calendarEvent.findUnique({
        where: { id: eventId },
...
export const getOccurrence = async (req, res, next) => {
    try {
        ...
        if (exception?.action === 'skip')
            return next(new AppError('This occurrence has been removed', 404));
    ...
    try {
        ...
        const updated = await prisma.$transaction(async (tx) => {
            const overrideData = {
                ...
                meetingLinkTitle: meetingLinkUrl !== undefined ? (meetingLinkUrl ? meetingLinkTitle || null : null) : event.meetingLinkTitle,
                meetingLinkPlatform: meetingLinkUrl !== undefined
                    ? meetingLinkUrl
            ...
            if (guests !== undefined) {
                ...
                if (guests.length) {
                    await tx.guest.createMany({
                        data: guestsCreateData(guests).map((g) => ({ ...g, eventId: overrideEventId })),
                    });
                }
            }
            if (reminders !== undefined) {
                ...
                if (reminders.length) {
                    await tx.eventReminder.createMany({
                        data: remindersCreateData(reminders).map((r) => ({ ...r, eventId: overrideEventId })),
                    });
                }
            }
            ...
            return tx.calendarEvent.findUnique({ where: { id: overrideEventId }, include: EVENT_INCLUDE });
        });
        res.status(200).json({
...
export const skipOccurrence = async (req, res, next) => {
    try {
        const event = await loadRecurringMasterForOccurrence(Number(req.params.eventId), next, {
            guests: { select: { userId: true } },
            recurrence: true,
        ...
        if (existing?.overrideEventId) {
            ...
            if (overrideEvent)
                await Promise.allSettled(overrideEvent.attachments.map((a) => destroyBlob(a)));
        }
