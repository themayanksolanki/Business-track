...
export const canAccessMeeting = (user, meeting) => {
    ...
    if (meeting.hostId === user.id)
        return true;
    if (meeting.participants.some((p) => p.userId === user.id))
        return true;
    return meeting.settings?.allowGuestJoin ?? false;
};
export const canEditMeeting = (user, meeting) => user.role === 'Admin' || meeting.hostId === user.id;
export const canEndMeeting = (user, meeting, participants) => canEditMeeting(user, meeting) ||
...
export const leaveMeeting = async (req, res, next) => {
    try {
        ...
        if (!participant)
            return next(new AppError('You are not a participant in this meeting', 404));
...
export const endMeeting = async (req, res, next) => {
    try {
        ...
        if (!meeting)
            return next(new AppError('Meeting not found', 404));
        if (!canEndMeeting(req.user, meeting, meeting.participants))
...
export const updateMeeting = async (req, res, next) => {
    try {
        ...
        if (!meeting)
            return next(new AppError('Meeting not found', 404));
        if (!canEditMeeting(req.user, meeting))
...
export const getMeetingHistory = async (req, res, next) => {
    try {
        ...
        const participants = meeting.participants.map((p) => ({
            ...p,
            durationSeconds: p.joinedAt && p.leftAt ? Math.round((p.leftAt.getTime() - p.joinedAt.getTime()) / 1000) : null,
        }));
