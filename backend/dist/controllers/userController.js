...
export const activateUser = async (req, res, next) => {
    try {
        ...
        const canActivate = canManageRole(currentRole, targetRole) &&
            (currentRole === 'Admin' ||
...
export const getReassignableWork = async (req, res, next) => {
    try {
        const target = await prisma.user.findUnique({ where: { id: Number(req.params.id) } });
        if (!target || !sameOrg(target.organizationId, req.user.organizationId))
            return next(new AppError('User not found', 404));
...
export const updateUserPassword = async (req, res, next) => {
    try {
        ...
        const targetRole = target.role;
        const allowed = canManageRole(callerRole, targetRole) &&
            (callerRole === 'Admin' ||
                callerRole === 'Manager' ||
