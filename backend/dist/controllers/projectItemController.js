...
const shiftDescendantDepths = async (rootId, delta) => {
    ...
    while (frontier.length) {
        ...
        if (children.length === 0)
            break;
        await prisma.$transaction(children.map((c) => prisma.projectItem.update({
            where: { id: c.id },
...
export const getItemsSummary = async (req, res, next) => {
    try {
        ...
        for (const a of imageAttachments) {
            ...
            if (!summary[key])
                summary[key] = { commentCount: 0, cover: null };
...
export const createItem = async (req, res, next) => {
    try {
        ...
        if (parent)
            await recomputeAncestorStatuses(parent.id);
...
export const updateItem = async (req, res, next) => {
    try {
        ...
        if (assignedTo !== undefined)
            data.assignedToId = assignedToNum;
    ...
    try {
        ...
        const duplicate = await prisma.$transaction(async (tx) => {
            if (laterSiblings.length) {
                await Promise.all(laterSiblings.map((s) => tx.projectItem.update({ where: { id: s.id }, data: { order: s.order + 1 } })));
            }
            return duplicateSubtree(tx, item, project.id, project.organizationId, item.parentId, item.order + 1, req.user.id, true);
        });
        if (item.parentId)
            await recomputeAncestorStatuses(item.parentId);
    ...
    try {
        ...
        if (direction === 'up' || direction === 'down') {
            ...
            if (swapWith < 0 || swapWith >= siblings.length)
                return next(new AppError(`Item is already at the ${direction === 'up' ? 'top' : 'bottom'}`, 400));
            ...
            if (toClose.length)
                await prisma.$transaction(toClose.map((s, i) => prisma.projectItem.update({ where: { id: s.id }, data: { order: index + i } })));
            ...
            if (depthDelta !== 0)
                await shiftDescendantDepths(item.id, depthDelta);
            ...
            if (!oldParentId)
                return next(new AppError('Item is already at the top level', 400));
            const oldParent = await prisma.projectItem.findUnique({ where: { id: oldParentId } });
            if (!oldParent)
                return next(new AppError('Parent item not found', 404));
            ...
            if (oldSiblings.length)
                await prisma.$transaction(oldSiblings.map((s, i) => prisma.projectItem.update({ where: { id: s.id }, data: { order: i } })));
            ...
            if (toShift.length)
                await prisma.$transaction(toShift.map((s) => prisma.projectItem.update({ where: { id: s.id }, data: { order: s.order + 1 } })));
            ...
            await recomputeAncestorStatuses(oldParentId);
            if (newParentId)
                await recomputeAncestorStatuses(newParentId);
        }
...
export const moveItemToProject = async (req, res, next) => {
    try {
        const project = await prisma.project.findUnique({
            where: { id: Number(req.params.projectId) },
            include: ACCESS_INCLUDE_WITH_ROLE,
        ...
        if (item.type === 'group') {
            ...
            if (!targetParentId)
                return next(new AppError('targetParentId is required for this item', 400));
            newParent = await prisma.projectItem.findFirst({
                where: { id: Number(targetParentId), projectId: targetProjectId },
            });
            if (!newParent)
                return next(new AppError('Target item not found in the target project', 404));
            newDepth = newParent.depth + 1;
        }
        const subtreeMaxDepth = await getMaxDescendantDepth(item.id, item.depth);
        const depthDelta = newDepth - item.depth;
...
export const bulkMoveItemsToParent = async (req, res, next) => {
    try {
        ...
        if (!newParent)
            return next(new AppError('Target group not found in this project', 404));
        const newDepth = newParent.depth + 1;
        const items = await prisma.projectItem.findMany({
        ...
        for (const id of affectedParentIds)
            await recomputeAncestorStatuses(id);
        res.status(200).json({
...
export const bulkMoveItemsToProject = async (req, res, next) => {
    try {
        const project = await prisma.project.findUnique({
            where: { id: Number(req.params.projectId) },
            include: ACCESS_INCLUDE_WITH_ROLE,
        ...
        if (!newParent)
            return next(new AppError('Target group not found in the target project', 404));
        const newDepth = newParent.depth + 1;
        const items = await prisma.projectItem.findMany({
...
export const reorderItems = async (req, res, next) => {
    try {
        ...
        if (numericOrderedIds.length !== siblingIds.size ||
        ...
        await prisma.$transaction(numericOrderedIds.map((id, index) => prisma.projectItem.update({ where: { id }, data: { order: index } })));
...
//# sourceMappingURL=projectItemController.js.map