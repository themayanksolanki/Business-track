...
export const getDashboardStats = async (req, res, next) => {
    try {
        const user = req.user;
        const now = new Date();
        const taskWhere = { parentTaskId: null, organizationId: user.organizationId };
        if (user.role === 'User') {
        ...
        const projectWhere = { organizationId: user.organizationId };
        if (user.role !== 'Admin') {
        ...
        const result = { tasks, projects, recentProjects };
        if (user.role === 'Admin' || user.role === 'Manager') {
            const departmentWhere = { organizationId: user.organizationId };
