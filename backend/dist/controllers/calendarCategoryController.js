...
// management page.
export const getCalendarCategories = async (req, res, next) => {
    try {
...
export const createCalendarCategory = async (req, res, next) => {
    try {
...
export const updateCalendarCategory = async (req, res, next) => {
    try {
...
export const deleteCalendarCategory = async (req, res, next) => {
    try {
        const category = await prisma.calendarCategory.findUnique({ where: { id: Number(req.params.id) } });
        if (!category || category.organizationId !== req.user.organizationId)
            return next(new AppError('Category not found', 404));
...
//# sourceMappingURL=calendarCategoryController.js.map