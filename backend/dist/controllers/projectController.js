...
export const canManageProjectSettings = (user, project) => user.role === 'Admin' ||
    user.role === 'Manager' ||
...
export const canEditProject = (user, project) => {
    ...
    if (membership)
        return membership.role?.canEdit !== false;
    return true;
};
...
export const canApproveDraft = (user, project) => user.role === 'Admin' || project.createdById === user.id;
...
const parseCsvNumbers = (v) => typeof v === 'string' && v ? v.split(',').map(Number).filter((n) => !Number.isNaN(n)) : [];
const parseCsvEnum = (v, valid) => typeof v === 'string' && v ? v.split(',').filter((s) => valid.includes(s)) : [];
...
const buildProjectOrderBy = (sortBy, sortDir) => {
...
export const getProjects = async (req, res, next) => {
    try {
        ...
        else if (VALID_PROJECT_STATUSES.includes(req.query.status)) {
        ...
        if (efforts.length)
            where.effort = { in: efforts };
        const applyDateRange = (fromParam, toParam, field) => {
            const from = req.query[fromParam];
            const to = req.query[toParam];
            if (!from && !to)
                return;
            where[field] = {
...
export const updateProject = async (req, res, next) => {
    try {
        ...
        const { name, description, startDate, endDate, owner, priority, department, category, status, detailsText, effort, links, tags, } = req.body;
        ...
        if (owner !== undefined)
            data.ownerId = owner ? Number(owner) : null;
        ...
        if (detailsText !== undefined)
            data.detailsText = detailsText;
        if (effort !== undefined)
            data.effort = effort;
        if (links !== undefined)
            data.links = links.map((l) => ({ title: l.title.trim(), url: l.url.trim() }));
        if (tags !== undefined)
...
export const downloadProjectPlan = async (req, res, next) => {
    try {
        ...
        if (!(await canAccessProject(req.user, project)))
            return next(new AppError('You do not have access to this project', 403));
        if (!project.planUrl)
            return next(new AppError('No plan has been uploaded', 404));
        let viewUrl;
        let downloadUrl;
