...
export const getMyOrganization = async (req, res, next) => {
    try {
        if (!req.user.organizationId)
            return next(new AppError('You are not part of an organization', 404));
...
export const createInvite = async (req, res, next) => {
    try {
        ...
        if (existingInvite)
            return next(new AppError('An invite for that email is already pending', 409));
        let deptIds = [];
...
export const acceptInviteByToken = async (req, res, next) => {
    try {
        const { username, password } = req.body;
        const invite = await prisma.invite.findUnique({
            where: { token: req.params.token },
