...
async function loadAccessibleMetric(req, next) {
...
export const savePeriodDiff = async (req, res, next) => {
    try {
        const metric = await loadAccessibleMetric(req, next);
        if (!metric)
            return;
        ...
        const doc = await MetricTracking.findOneAndUpdate({ metricId: metric.id, frequency, year, month }, { $setOnInsert: { metricId: metric.id, frequency, year, month } }, { upsert: true, new: true });
        ...
        for (const [key, value] of Object.entries(diff)) {
            ...
            const existingObj = existing && typeof existing.toObject === 'function' ? existing.toObject() : existing ?? {};
        ...
        for (const value of doc.periods.values()) {
            if (typeof value.actual === 'number')
                actualTotal += value.actual;
            if (typeof value.target === 'number')
                targetTotal += value.target;
        }
        ...
        await prisma.metricPeriodTotal.upsert({
            ...
            where: { metricId_frequency_year_month: { metricId: metric.id, frequency, year, month: month } },
