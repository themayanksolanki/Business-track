...
export async function loadOrgTask(taskId, user) {
    const task = await prisma.task.findUnique({ where: { id: taskId } });
    if (!task || task.organizationId !== user.organizationId)
        throw new AppError('Task not found', 404);
    return task;
}
export async function loadOrgTaskWithRelations(taskId, user) {
...
export async function requireTaskEditAccess(task, user, message = 'Access denied') {
