...
export const getS3DownloadUrl = ({ key, mimeType, fileName, disposition = 'inline', expiresIn = 300, }) => getSignedUrl(s3Client, new GetObjectCommand({
    Bucket: S3_BUCKET,
    Key: key,
    ResponseContentType: mimeType,
    ResponseContentDisposition: `${disposition}; filename="${encodeURIComponent(fileName)}"`,
}), { expiresIn });
export const deleteS3Object = (key) => s3Client.send(new DeleteObjectCommand({ Bucket: S3_BUCKET, Key: key }));
