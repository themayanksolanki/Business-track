...
export const ROLE_RANK: Record<string, number> = { Admin: 4, Manager: 3, 'Team Lead': 2, User: 1 };

type AuthUser = Pick<PrismaUser, 'id' | 'role' | 'organizationId'>;
...
export const canManageRole = (actorRole: string, targetRole: string) =>
...
export const canActOnUser = (actor: AuthUser, target: Pick<PrismaUser, 'role' | 'managerId' | 'teamLeadId'>) => {
  if (!canManageRole(actor.role, target.role)) return false;
  if (actor.role === 'Admin') return true;
  if (actor.role === 'Manager') return target.managerId === actor.id;
  if (actor.role === 'Team Lead') return target.teamLeadId === actor.id;
  return false;
};
...
// full walk per root.
export const getDescendantIds = async (rootIds: number | number[]): Promise<number[]> => {
  const result: number[] = [];
...
export const getAccessibleDepartmentIds = async (user: AuthUser): Promise<number[] | null> => {
...
export const canAccessDepartment = (accessibleIds: number[] | null, departmentId: number | string) =>
...
export const getTeamMemberIds = async (teamLeadId: number): Promise<number[]> => {
  const members = await prisma.user.findMany({ where: { teamLeadId, role: 'User' }, select: { id: true } });
  return members.map((m) => m.id);
};

type AccessTask = Pick<PrismaTask, 'organizationId' | 'assignedToId' | 'createdById'>;
...
export const getTaskAccessLevel = async (task: AccessTask, user: AuthUser): Promise<'edit' | null> => {
