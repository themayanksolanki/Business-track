...
// week.
function* candidateDates(rule: RecurrenceRuleLike, eventStart: Date): Generator<Date> {
  const interval = Math.max(1, rule.interval || 1);

  if (rule.frequency === 'weekly') {
    const weekStart = startOfWeek(eventStart); // Sunday = 0, matches byWeekday's convention
    const weekdays = Array.from(new Set(rule.byWeekday.length ? rule.byWeekday : [eventStart.getDay()])).sort(
      (a, b) => a - b
    );
...
export function isGeneratedOccurrence(
  ...
  const probe = generateOccurrences(
    rule,
    eventStart,
    eventEnd,
    new Date(originalStart.getTime() - 1),
    new Date(originalStart.getTime() + 1)
  );
