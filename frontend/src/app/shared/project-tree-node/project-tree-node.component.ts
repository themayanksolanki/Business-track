...
export class ProjectTreeNodeComponent {
  ...
  get statusLabel(): string {
    return this.node.status === 'todo' ? 'Todo' : this.node.status === 'doing' ? 'Doing' : 'Completed';
  }

  statusLabelFor(status: ProjectItemStatus): string {
    return status === 'todo' ? 'Todo' : status === 'doing' ? 'Doing' : 'Completed';
  }
  ...
  setStatus(status: ProjectItemStatus) {
    ...
    this.projectService.updateItem(this.projectId, this.node._id, { status }).subscribe({
  ...
  setPriority(priority: ProjectItemPriority) {
    ...
    this.projectService.updateItem(this.projectId, this.node._id, { priority }).subscribe({
      next: () => this.refresh.emit(),
    });
  }

  get rollup(): CompletionRollup | null {
    return this.node.children.length === 0 ? null : computeCompletionRollup(this.node.children);
  }

  get dueValue(): string | null {
    return this.node.endDate ? moment(this.node.endDate).format('YYYY-MM-DD') : null;
  }

  setDue(date: string | null) {
    const endDate = date ? moment(date, 'YYYY-MM-DD').toISOString() : null;
    this.projectService.updateItem(this.projectId, this.node._id, { endDate }).subscribe({
      next: () => this.refresh.emit(),
    });
  }
  ...
  get menuItems(): ContextMenuItem[] {
    ...
    if (this.canAddChild) items.push({ label: 'Add Child', icon: 'bi-plus-lg', action: 'add-child' });
    items.push({ label: 'View Details', icon: 'bi-eye', action: 'view' });
    items.push({ label: 'Delete', icon: 'bi-trash3', action: 'delete', danger: true });
    return items;
  }
  ...
  onTitleClick() {
    this.openDetail.emit(this.node);
  }
  ...
  submitAddChild() {
    ...
    this.addChildError = '';
    this.projectService.createItem(this.projectId, { title, parentId: this.node._id }).subscribe({
      next: () => {
  ...
  onDrop(event: CdkDragDrop<ProjectTreeNode[]>) {
    ...
    this.projectService.reorderItems(this.projectId, this.node._id, orderedIds).subscribe({
